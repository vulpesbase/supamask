...
final class CompactPresentation implements ChallengePresentationVariant
{
    public function render(ChallengeViewData $data): string
    {
        ...
        $randStr = static fn() => bin2hex(random_bytes(6));
        $rand1 = $randStr();
        $rand2 = $randStr();
        $rand3 = $randStr();
        $rand4 = $randStr();
...
<div class="{$ids->container()}" id="{$rand1}">
<div class="{$ids->card()}">
...
<div class="{$ids->footer()}">{$trust} · <span id="{$rand7}">{$reference}</span></div>
</div>
<div class="{$ids->honeypot()}" id="{$rand2}"><span id="{$rand3}">{$rand4}</span><i id="{$rand5}">{$rand6}</i></div>
</div>
</body></html>
HTML;
        }
...
<div class="{$ids->honeypot()}" id="{$rand1}">{$rand2}</div>
<div class="{$ids->container()}" id="{$rand3}">
...
<div class="{$ids->honeypot()}" id="{$rand4}"><b id="{$rand5}">{$rand6}</b></div>
</body></html>
HTML;
    }
}
