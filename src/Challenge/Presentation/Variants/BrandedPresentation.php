...
final class BrandedPresentation implements ChallengePresentationVariant
{
    public function render(ChallengeViewData $data): string
    {
        ...
        // Generate some random non-semantic strings for the honeypots
...
<div class="{$ids->honeypot()}" id="{$rand1}" data-x="{$rand2}"></div>
<div class="{$ids->card()}">
...
<div class="{$ids->footer()}"><span>{$trust}</span><span>ref <span id="{$rand6}">{$reference}</span></span></div>
</div>
</div>
<div class="{$ids->honeypot()}" id="{$rand3}"><span id="{$rand4}">{$rand5}</span></div>
</div>
</body></html>
HTML;
    }
